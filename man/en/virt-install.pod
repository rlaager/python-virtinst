...
B<virt-install> is a command line tool for provisoning new virtual machines
...
OS is known to be incompatiable with the accelerators. The KVM accelerator is
preferred over KQEMU if both are available.

=item  --connect=CONNECT     Connect to hypervisor with URI

Connect to a non-defautl hypervisor. The default connection is chosen from
...
Tailor the guest configuration to suit a specific installation method. There
are two supported installers:

=over 4

=item distro

This is the default installer. Paravirtualized guests will be started with a
a kernel+initrd pair, and after installation will use the C<pygrub> bootloader.
Fullyvirtualized guests will be started with a CDROM device attached, and after
installation will revert to booting off the primary harddisk.

=item livecd

This is only available for fullyvirtualized guests. The guest wil be permanently
configured to boot off the CDROM device.
...
path to an ISO image, or to a CDROM device. This parameter is deprecated, with 
the C<--location> parameter as the recommended replacement.
...
enable use of acceleration. If omited, the host CPU architecture will
...
boot off an CDROM. The C<LOCATION> can take one of the following forms:

=over 4

=item DEVICE

Path to a CDROM device for fullyvirtualized guests

=item FILE

Path to an ISO image for fullyvirtualized guests
...
An NFS server location containing an installable distribution image for
either full or paravirtualized guests.

=item http://host/path

An HTTP server location containing an installable distribution image for
either full or paravirtualized guests.

=item ftp://host/path

An FTP server location containing an installable distribution image for
either full or paravirtualized guests.
...
The debugging information is also stored in C<$HOME/.virt-install/virt-install.log>
