...
class TestXenConfig(unittest.TestCase):

    def _compare(self, xenguest, filebase):
        f = open(filebase + ".xen", "r")
        expectXen = string.join(f.readlines(), "")
        f.close()

        actualXen = xenguest._get_config_xen()

        if os.environ.has_key("DEBUG_TESTS") and os.environ["DEBUG_TESTS"] == "xen":
            print "Actual: %d bytes '%s'" % (len(actualXen),  actualXen)
            print "Expect: %d bytes '%s'" % (len(expectXen),  expectXen)

        self.assertEqual(actualXen, expectXen)
